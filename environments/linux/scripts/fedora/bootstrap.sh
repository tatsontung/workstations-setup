...
then
    ...
    sudo dnf install -y git build-essential vim vim-gtk3 neovim wget curl htop lsof tilix tmux tty-clock \
    fzf fasd zsh fish taskwarrior copyq ranger mc cmus httpie tree xsel samba-common exfat-utils \
    nginx python python3 python3-pip ruby memcached docker docker-compose python3-pip pkg-config screenfetch \
    libsecret-1-0 libsecret-1-dev atool highlight w3m mediainfo vim-gtk fonts-noto-color-emoji emacs cmake libvterm-dev
